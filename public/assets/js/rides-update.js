    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#addStop', function(e) {
            var htmlTemplate = '<div id="inputForAddStop">' +
                ...
                'שעת איסוף ' +
                '</label>' +
                '<input class="form-control input-time stop-time" class="form-control" name="addresses[stopTime]" placeholder="13:45">' +
                '<label for="addresses">' +
                'איש קשר לתחנה זו' +
                '</label>' +
        ...
        $("#ride-update").on('keyup', '.stop-time', function() {

            var currentStop = Number($(this).val().replace(':', ''));
            var elemntsStartTime = $('.start-time');
            var elemntsEndTime = $('.end-time');
            var generalStartTime, generalEndTime;

            if ($(elemntsStartTime[0]).val() !== "") {
                generalStartTime = Number($(elemntsStartTime[0]).val().replace(':', ''));
                generalEndTime = Number($(elemntsEndTime[0]).val().replace(':', ''));
            }
            else {
                generalStartTime = Number($(elemntsStartTime[1]).val().replace(':', ''));
                generalEndTime = Number($(elemntsEndTime[1]).val().replace(':', ''));
            }
            console.log(currentStop, generalStartTime, generalEndTime);

            if (currentStop > generalEndTime || currentStop < generalStartTime) {
