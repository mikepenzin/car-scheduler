...
$(document).ready(function() {
	...
	function appendVendor(vendor) {
		...
		if (vendor.isActive) {
			vendorActive = '<div id="not-active-' + vendor._id + '" class="text-warning no-driver-message mt-2 d-none d-sm-block" style="visibility: hidden;">';
			...
			vendorActive = '<div id="not-active-' + vendor._id + '" class="text-warning no-driver-message mt-2 d-block">';
		...
		var vendorTemplate = $('<div id="general_' + vendor._id + '" class="col-12 col-sm-10 col-md-6 col-lg-6 col-xl-6">' +
			'<div class="card">' +
			'<div class="card-info">' +
			...
			.then(function(updatedVendor) {
				...
				if (result.isActive) {
					...
					$('#not-active-' + id).animate({ opacity: 0.0 }, 500, function() {
						$('#not-active-' + id).addClass("d-sm-block d-none").removeClass("d-block").css("visibility", "hidden");
					});
				}
				else {
					element.html(' הפעלה <i class="fal fa-eye ml-1">');
					$('#not-active-' + id).removeClass("d-sm-block d-none").addClass("d-block").css({ visibility: "visible", opacity: 0.0 }).animate({ opacity: 1.0 }, 500);
				}
